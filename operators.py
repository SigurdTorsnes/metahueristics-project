...
def smart_one_insert(sol):
    ...
    p_random_remove = data.num_calls/(data.num_calls*3+dummy_amount*2)
    if random.random() <= p_dummy_removal:
        call = random.choice(dummy_calls)
    elif random.random() <= p_random_remove:
        call = random.randint(1,data.num_calls)
    else:
        ...
        call = helpers.find_k_expensive_calls(solution)[0]

    # find insertion vehicle:
    vehicle_id = helpers.pick_random_vehicle_by_call(call) # insertion_vehicle
...
def insert_from_dummy(sol):
    solution = sol[:]
    dummy_index = helpers.find_dummy_index(solution)
    dummy_calls = list(set(solution[dummy_index:]))
    vehicle_start_index,vehicle_end_index,vehicle_id = helpers.pick_good_vehicle_insertion(solution)
...
def smart_k_insert(sol):
    ...
    for i in range(k):
        ...
        vehicle_id = helpers.pick_random_vehicle_by_call(call)
        vehicle_start_index,vehicle_end_index = helpers.find_vehicle_indices(solution,vehicle_id)
        vehicle_calls = solution[vehicle_start_index:vehicle_end_index]
        if vehicle_start_index==vehicle_end_index:
            solution = helpers.reinsert_at_index(solution,call,vehicle_end_index)
